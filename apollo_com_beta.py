...
class ApolloCommUI(object):
    def __init__(self):
        ...
        self.file_io(mode='r')
        self.file_name_list = ['AOA_cal_step_param', 'dll_libs', 'pm2_step_param', 'station_cal_step_param',
                               'station_param', 'station_script', 'test_results']

    def menu_list(self):
        Label(self.tk, text='Apollo IP addr:').grid(row=1, column=1)
    ...
    def build_folders(self):
        tkMessageBox.showwarning(title='Warning', message='Are you sure you want to build file path?'
                                                          'All existing files will be replaced!')
        try:
            os.mkdir('C:\pm2_config')
            for elements in self.file_name_list:
                os.mkdir('C:\pm2_config\{}'.format(elements))
        except OSError as e:
    ...
    def parse_shutdown(self):
        return tkMessageBox.askyesno('Shut down?')
