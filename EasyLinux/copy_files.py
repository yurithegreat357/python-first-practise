...
class CopyFile(object):
    ...
    def menu_list(self):
        ...
        self.but_update_group()
        # self.but_ping_server_status()
        mainloop()
...
#   ===============Buttons definitions under===================================

    def but_update_group(self):
        Button(self.tk, text='update group', command=self.update_local_group, bg='gold').grid(row=1, column=2)
    ...
    # def but_ping_server_status(self):
    #     Button(self.tk, text='Ping server status', command=self.ping_server_status).grid(row=4, column=1)
#   ===============Buttons definitions end===================================
    ...
    def ping_server_status(self):
        ...
        if group == 'Antigua':
            self.ping_server_process(copy_update_config.antigua_machine)
        elif group == 'Barbados':
            self.ping_server_process(copy_update_config.barbados_machine)
        else:
            showwarning(message='Select your product group first!!')

    def ping_server_process(self, server_lst):
        for number in range(1, len(server_lst)):
            ret = subprocess.call(['ping', server_lst[number]])
            if ret == 0:
                self.new_windows(info='{0} is up'.format(server_lst[number]))
            else:
                self.new_windows(info='{0} is not online'.format(server_lst[number]))
        showwarning(title='Ping status', message='Ping servers complete!')
    ...
    def copy_commands(self, des_machine):
        ...
        if result:
            ...
            index = child.expect(['.*(yes/no)?', pexpect.EOF], timeout=40)
            if index == 0:
                child.sendline('yes')
                child.sendline('\r')
            else:
